...
// IMAP configuration (fixed credentials)
const IMAP_CONFIG = {
  imap: {
    user: "",
...
app.post("/fetch-latest-email", async (req, res) => {
  ...
  try {
    IMAP_CONFIG.imap.user = email; // Dynamically set the user email
    ...
    if (messages.length === 0) {
      connection.end();
      return res.status(404).json({ error: "No emails with subject 'Netflix' found" });
    }

    messages.sort((a, b) => b.attributes.date - a.attributes.date); // Sort messages by date to get the latest
